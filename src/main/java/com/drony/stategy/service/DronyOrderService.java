package com.drony.stategy.service;
...
public class DronyOrderService {
  ...
  public boolean createOrder(IBar bar, IBar prevBar, DirectionEnum direction,
    ...
    if (direction.equals(DirectionEnum.BUY)) {
      price = bar.getClose() + (Math.max(
          ((this.paramDrony.getIndentPercetPennachio() / 100) * (bar.getHigh() - bar.getClose())),
          0)
          + identFixed);
      price = Utility.roundByDefaultPrecision(price, instrument);
    } else {
      price = bar.getClose() - (Math.max(
          ((this.paramDrony.getIndentPercetPennachio() / 100) * (bar.getClose() - bar.getLow())), 0)
          + identFixed);
      price = Utility.roundByDefaultPrecision(price, instrument);
    }
    ...
    try {
      ...
      com.drony.stategy.data.DronyOrder dronyOrder = new com.drony.stategy.data.DronyOrder(
          ...
          instrument,
          this.outPutVerboso); // TODO ASK ARDUINO come primo prezzo di riferimento prendo il prezzo di chiusura delle barre +/- ident

      if (delegateDrony.testAndAddOrterToCluster(this.paramDrony.getOrderCluster(), orderLabel,
  ...
  private String getLabel(long time) {
    return this.identifier + "_" + DronyOrderService.ORDER_DATE_FORMAT.format(time) + Utility
        .generateRandom(10000)
        + Utility.generateRandom(10000);
  }
